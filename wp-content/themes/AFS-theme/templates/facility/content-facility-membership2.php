...
<h3>membership type</h3>
	...
	<?php if ( $fields['membership_type'] && time() < $fields['membership_expiration'] ) : ?>
        ...
        <p>Your subscription renews on <?php echo date('m/d/Y', $fields['membership_expiration']); ?> for <?php echo '$'.SF_Users::$facility_membership_types[$fields['membership_type']]['cost']; ?>. <a target="_blank" href="https://www.paypal.com/us/cgi-bin/webscr?cmd=_manage-paylist">manage your subscription at Paypal.</a></p>
       
       <?php 
	   //Check for addons
	   $existing_addons = fv_get_facility_membership_addons($facility_id);
	   if ( $existing_addons ) {
		   	echo '<h4>active add-ons</h4>';
			foreach ( $existing_addons as $addon) {
				if ( $addon['addon'] && time() < $addon['expiration'] ) {
					$addon_label = SF_Users::$facility_membership_types[$addon['addon']]['description'];
					?>
       ...
       <?php 
	   //Show addons
	   foreach ( SF_Users::$facility_membership_types as $each_key => $each ) : 
	   	if ( $each['type'] == 'addon' ) :
	   		?>
       ...
       <p><img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/img/paypal-logo.png"></p>
         ...
         <?php if ( isset($_GET['member_addon']) ) : ?>
            ...
            <?php
			 ...
			 $button = '<button class="btn append large orange"><i class="fa fa-thumbs-up border-right"></i>'.'$'.SF_Users::$facility_membership_types[$new_membership]['cost'].'/yr. Add-on</button>';
			 $label = SF_Users::$facility_membership_types[$new_membership]['description'];
         	 $subscribe_form = SF_Users::get_new_facility_subscription_form(get_current_user_id(), $facility_id, $button, $new_membership);
			 ?>
...
<?php else : ?>
    ...
    $new_membership = 'F1'; //main
    $button = '<button class="btn append large orange"><i class="fa fa-thumbs-up border-right"></i>'.'$'.SF_Users::$facility_membership_types[$new_membership]['cost'].'/yr. Paid Membership</button>';
    $subscribe_form = SF_Users::get_new_facility_subscription_form(get_current_user_id(), $facility_id, $button, $new_membership);
    ?>
