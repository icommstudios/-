...
if ( $_GET['add_endorsement'] ) {
	...
	$existing_endorsement = SF_Project::get_field( $project_id, 'endorsement_id_by_facility');
}
?>
<?php if ( $project_id ) : ?>

	<?php if ( ($project_facility_id != $facility_id) ) : //check logged in user ?>
    ...
    <p>Please rate your experience with <strong><?php echo get_the_title($project_contractor_id); ?></strong>. (1 is bad, 5 is great)</p>
    ...
    <?php comment_id_fields($project_contractor_id); ?>
    <?php do_action('comment_form', $project_contractor_id); ?>
    </form>
